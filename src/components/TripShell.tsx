...
export function TripShell() {
    ...
    if (
      trip.expenses.some(
        (e) =>
          e.paidBy === id ||
          e.allocations.some(
            (a) => a.participantId === id && a.value > 0
          )
      )
    ) {
      return alert(
        "This participant is used by an expense. Edit/remove those expenses first."
      );
    }
  ...
  function backup() {
    const blob = new Blob([exportTrip(trip)], {
      type: "application/json",
    });

    const a = document.createElement("a");
    a.href = URL.createObjectURL(blob);
    a.download = `${trip.name.replace(/\W+/g, "-")}.json`;
    a.click();
  ...
  function restore() {
    ...
    input.onchange = async () => {
      ...
      try {
        replaceTrip(importTrip(await f.text()));
      } catch {
  ...
  const nav = [
    ...
    ["settlements", "Settlements", CreditCard],
    ["people", `People (${trip.participants.length})`, Users],
  ] as const;
    ...
    <div className="min-h-screen bg-slate-50 text-slate-950 dark:bg-slate-950 dark:text-slate-100">
      <header className="sticky top-0 z-30 border-b border-slate-200/80 bg-white/90 backdrop-blur dark:border-slate-800 dark:bg-slate-950/90">
        <div className="mx-auto flex max-w-6xl items-center gap-3 px-4 py-3">
          <div className="grid h-9 w-9 place-items-center rounded-xl bg-slate-900 font-black text-white dark:bg-white dark:text-slate-900">
            TS
          </div>

          <div className="min-w-0 flex-1">
            <p className="truncate font-black">{trip.name}</p>
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-slate-500">
            Manage everyone sharing the trip.
          </p>
        </div>

        <Button
          onClick={onAdd}
          disabled={trip.participants.length >= 50}
        >
          Add participant
        </Button>
      </div>
      ...
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
        {trip.participants.map((p: any) => (
          ...
          >
            <div className="grid h-10 w-10 place-items-center rounded-full bg-slate-100 font-bold dark:bg-slate-800">
              {p.name[0]}
            </div>
      ...
      <div className="rounded-2xl border border-dashed border-slate-300 p-5 dark:border-slate-700">
        <p className="font-bold">Backup & restore</p>

        <p className="mt-1 text-sm text-slate-500">
          Restore a JSON backup from another browser/device.
        </p>
